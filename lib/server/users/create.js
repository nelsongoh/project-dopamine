  ...
  try {
    ...
    if (userDetails.hasOwnProperty("permissions")) {
      ...
      if (userPerms.length === 0) {
        // If there are no permissions selected for the user, throw an error
        creationErrors.email.hasError = true;
      ...
      if (permsDoc.exists) {
        ...
        if (validUserPerms.length === 0) {
          // There are no valid permissions for the user found, we throw an error
          creationErrors.email.hasError = true;
      ...
      // If there isn't a "permissions" key, we throw an error
      creationErrors.email.hasError = true;
